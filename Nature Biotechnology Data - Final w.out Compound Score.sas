...
data wCombinedFile;
	set zCombinedFile;
		...
		wWEF_Perc_IP_Protection = nWEF_Perc_IP_Protection * .20;
		wShare_World_Biotech_Patents = nShare_World_Biotech_Patents * 0;
		wYear5_Trend_Share_Biotech = nYear5_Trend_Share_Biotech * 0;
		wCompound_Score_US_CoC = nCompound_Score_US_CoC * 0;
		wResTech_per_Mill = nResTech_per_Mill * .20;
		wPatent_File_Residents_M_Pop = nPatent_File_Residents_M_Pop * .10;
		wPatents_per_M_Pop_2018 = nPatents_per_M_Pop_2018 * .10;
		wPerc_Biotech_Patents_2018 = nPerc_Biotech_Patents_2018 * 0;
		wPerc_LifeScience_Patents_2018 = nPerc_LifeScience_Patents_2018 * .20;
		wNature_LS_TotalShare = nNature_LS_TotalShare * .10;
		wNature_Delta_LS_2019 = nNature_Delta_LS_2019 * .10;
...
/* Step 4: Calculate weighted score for an area
	- sum nationa's weighted values for ranking indicators in that area
...
data wsCombinedFile;
	...
	if (not missing(wWEF_Perc_IP_Protection) and not missing(wResTech_per_Mill) and not missing(wPatent_File_Residents_M_Pop)
		and not missing(wPatents_per_M_Pop_2018) and not missing(wPerc_LifeScience_Patents_2018)  and not missing(wNature_LS_TotalShare) and not missing(wNature_Delta_LS_2019))
			then IPProtectionSum = (wWEF_Perc_IP_Protection + wResTech_per_Mill + wPatent_File_Residents_M_Pop + wPatents_per_M_Pop_2018 + wPerc_LifeScience_Patents_2018 + wNature_LS_TotalShare + wNature_Delta_LS_2019);
	else IPProtectionSum = .;
run;

proc print data=wsCombinedFile;
run;
...
proc export 
 data=zwsCombinedFile
 dbms=xlsx 
 outfile="H:\[Client] Springer Nature\[2021.04] World View Data - Biotechnology Project\Nature Biotechnology Files\Weighted FinalRank Group 1 w.out Compound &sysdate..xlsx" 
 replace;
run;

/* Group B removes BiotechInv_TotalInv and Enterprise Variables */
...
proc export 
 data=g2zwsCombinedFile
 dbms=xlsx 
 outfile="H:\[Client] Springer Nature\[2021.04] World View Data - Biotechnology Project\Nature Biotechnology Files\Weighted FinalRank Group 2 w.out Compound &sysdate..xlsx" 
 replace;
run;
...
data uwCombinedFile;
	...
	if (not missing(nWEF_Perc_IP_Protection)and not missing(nResTech_per_Mill) and not missing(nPatent_File_Residents_M_Pop)
		and not missing(nPatents_per_M_Pop_2018) and not missing(nPerc_LifeScience_Patents_2018) and not missing(nNature_LS_TotalShare) and not missing(nNature_Delta_LS_2019))
			then IPProtectionSum = (nWEF_Perc_IP_Protection + nResTech_per_Mill + nPatent_File_Residents_M_Pop + nPatents_per_M_Pop_2018 + nPerc_LifeScience_Patents_2018 + nNature_LS_TotalShare + nNature_Delta_LS_2019);
	else IPProtectionSum = .;
run;
...
proc export 
 data=zuwCombinedFile
 dbms=xlsx 
 outfile="H:\[Client] Springer Nature\[2021.04] World View Data - Biotechnology Project\Nature Biotechnology Files\Unweighted FinalRanks Group 1 w.out Compound &sysdate..xlsx" 
 replace;
run;
...
proc export 
 data=g2zuwCombinedFile
 dbms=xlsx 
 outfile="H:\[Client] Springer Nature\[2021.04] World View Data - Biotechnology Project\Nature Biotechnology Files\Unweighted FinalRank Group 2 w.out Compound &sysdate..xlsx" 
 replace;
run;
